  ...
  methods: {
    ...
    getData () {
      ...
      getLtDetail({
        ...
        this.officialLsit = this.detailData.comments
        this.tagList = this.detailData.tags
        this.loading = true
        this.finished = false
        this.onLoad()
        if (this.detailData.img_dir) {
          this.imageList = this.detailData.img_dir.split(',')[0] ? this.detailData.img_dir.split(',') : this.baseUrl + this.detailData.img_dir
        }
      }).catch(err => {
        // 加载状态结束
        this.loading = false
